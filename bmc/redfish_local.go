...
func (r *RedfishLocalBMC) GetBMCAttributeValues(ctx context.Context, req GetBMCAttributeValuesRequest) (schemas.SettingsAttributes, error) {
	UUID, attributes := req.UUID, req.Attributes
	if len(attributes) == 0 {
		return nil, nil
	}
