...
func (r *BMCReconciler) reconcile(ctx context.Context, bmcObj *metalv1alpha1.BMC) (ctrl.Result, error) {
	...
	if err := r.updateBMCStatusDetails(ctx, bmcClient, bmcObj); err != nil {
		// If the failure is an HTTP error from the BMC (directly or wrapped in
		// a gofish CollectionError), classify it and update the Ready condition
		// so downstream logic (SSH reset, etc.) can react.
		httpErr := &schemas.Error{}
		var collErr *schemas.CollectionError
		if errors.As(err, &httpErr) || errors.As(err, &collErr) {
			// updateReadyConditionOnBMCFailure returns the original error verbatim
			// on success; only surface it if setting the condition itself failed.
			_ = r.updateReadyConditionOnBMCFailure(ctx, bmcObj, err)
...
func (r *BMCReconciler) updateReadyConditionOnBMCFailure(ctx context.Context, bmcObj *metalv1alpha1.BMC, err error) error {
	httpErr := &schemas.Error{}
	// gofish wraps per-item errors in a *schemas.CollectionError that does not
	// implement Unwrap, so peek inside its Failures map to recover the underlying
	// *schemas.Error and classify the failure.
	if !errors.As(err, &httpErr) {
		var collErr *schemas.CollectionError
		if errors.As(err, &collErr) {
			for _, failure := range collErr.Failures {
				if errors.As(failure, &httpErr) {
					break
				}
			}
		}
	}
	if httpErr != nil && httpErr.HTTPReturnedStatusCode != 0 {
