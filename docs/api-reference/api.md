...
| `failedAutoRetryCount` _integer_ | FailedAutoRetryCount is the number of times the controller should automatically retry the bios upgrade in case of failure before giving up. |  |  |
...
| `lastAppliedTime` _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.35/#time-v1-meta)_ | LastAppliedTime represents the timestamp when the last setting was successfully applied. |  |  |
| `autoRetryCountRemaining` _integer_ | AutoRetryCountRemaining is the number of remaining times the controller will automatically retry the bios upgrade in case of failure before giving up. |  |  |
