...
type BMCReconciler struct {
	...
	// DefaultDiscoveredServerReclaimPolicy sets spec.reclaimPolicy on newly discovered servers.
	// If empty, the API-level default (Recycle) applies.
	DefaultDiscoveredServerReclaimPolicy metalv1alpha1.ServerReclaimPolicy
}
...
func (r *BMCReconciler) discoverServers(ctx context.Context, bmcClient bmc.BMC, bmcObj *metalv1alpha1.BMC) error {
	...
	for i, s := range servers {
		...
		opResult, err := controllerutil.CreateOrPatch(ctx, r.Client, server, func() error {
			...
			if server.Spec.ReclaimPolicy == "" {
				server.Spec.ReclaimPolicy = r.DefaultDiscoveredServerReclaimPolicy
			}
