...
| `status.conditions[]` | Fine-grained checkpoints: version gate, maintenance waiting, reset, issue/verify results. | Primary source for error reason and where in workflow it failed. |
| `spec.serverMaintenanceRefs[]` | Requested or reused maintenance references per affected server. | Missing or stale refs explain prolonged maintenance wait behavior. |
