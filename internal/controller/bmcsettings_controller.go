...
func (r *BMCSettingsReconciler) shouldDelete(ctx context.Context, settings *metalv1alpha1.BMCSettings) bool {
	if settings.DeletionTimestamp.IsZero() {
		return false
	}

	if controllerutil.ContainsFinalizer(settings, BMCSettingFinalizer) &&
		settings.Status.State == metalv1alpha1.BMCSettingsStateInProgress {
		log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) getServerMaintenanceRefForServer(ServerMaintenanceRefs []metalv1alpha1.ServerMaintenanceRefItem, serverMaintenanceUID types.UID) *metalv1alpha1.ObjectReference {
	for _, serverMaintenanceRef := range ServerMaintenanceRefs {
...
func (r *BMCSettingsReconciler) enqueueBMCSettingsByServerRefs(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, settings := range settingsList.Items {
		...
		serverMaintenanceRef := r.getServerMaintenanceRefForServer(settings.Spec.ServerMaintenanceRefs, host.Spec.ServerMaintenanceRef.UID)
