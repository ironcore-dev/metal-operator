		...
		if currentSettingsFlowStatus == nil {
			...
			if errApplyInProgress != nil {
				...
				if len(settingDiff) == 0 {
					...
					err := r.updateBiosSettingsFlowStatus(ctx, log, biosSettings, metalv1alpha1.BIOSSettingsFlowStateApplied, currentSettingsFlowStatus, nil)
					if err != nil {
						return ctrl.Result{}, err
					}
				...
				err = r.handleFailedSettingFlowUpdate(ctx, log, biosSettings, errApplyInProgress)
				return ctrl.Result{}, err
			}
			return ctrl.Result{}, nil
		}
		...
		if currentSettingsFlowStatus.State != metalv1alpha1.BIOSSettingsFlowStateInProgress {
			...
			if currentSettingsFlowStatus.State == metalv1alpha1.BIOSSettingsFlowStateApplied {
				...
				if err != nil {
					...
					err := r.handleFailedSettingFlowUpdate(ctx, log, biosSettings, err)
					return ctrl.Result{}, err
				}
				return ctrl.Result{}, err
			}
...
func (r *BiosSettingsReconciler) handleFailedSettingFlowUpdate(ctx context.Context, log logr.Logger, biosSettings *metalv1alpha1.BIOSSettings, err error) error {
	if _, ok := err.(*AnotherFlowInProgressError); ok {
		for _, flowState := range biosSettings.Status.FlowState {
			if flowState.State == metalv1alpha1.BIOSSettingsFlowStateInProgress {
				...
				log.V(1).Info("Another BIOSSettingsFlow is already in progress. Waiting for completion", "SettingsName", flowState.Name, "Priority", flowState.Priority)
				errStatus := r.updateBiosSettingsStatus(ctx, log, biosSettings, metalv1alpha1.BIOSSettingsStateFailed, nil)
