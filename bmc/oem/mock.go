...
func (r *Mock) GetUpdateRequestBody(
...
func (r *Mock) GetUpdateTaskMonitorURI(response *http.Response) (string, error) {
	rawBody, err := io.ReadAll(response.Body)
	if err != nil {
		return "", fmt.Errorf("failed to read the response body %v %v", err, rawBody)
	}

	if taskMonitor, ok := response.Header["Location"]; ok {
		return taskMonitor[0], nil
	}

	return "", fmt.Errorf("unexpected response body %v %v", err, string(rawBody))
}

func (r *Mock) GetTaskMonitorDetails(ctx context.Context, taskMonitorResponse *http.Response) (*redfish.Task, error) {
	task := &redfish.Task{}
...
type MockServerAttributes struct {
	Id         string
...
type MockManagerLinksOEM struct {
	LinkAttributes  common.Links `json:"MockServerAttributes"`
	AttributesCount int          `json:"MockServerAttributesCount"`
}

func (d *MockManager) GetObjFromUri(
	ctx context.Context,
	uri string,
	respObj any,
) ([]string, error) {
...
func (d *MockManager) getCurrentBMCSettingAttribute(ctx context.Context) ([]MockServerAttributes, error) {

	type temp struct {
		OEMData MockManagerLinksOEM `json:"Contoso"`
	}
	...
	BMCDellAttributes := []MockServerAttributes{}
	var errs []error
	for _, data := range tempData.OEMData.LinkAttributes {
		BMCDellAttribute := &MockServerAttributes{}
...
func (d *MockManager) getFilteredBMCRegistryAttributes(
	ctx context.Context,
	readOnly bool,
	immutable bool,
) (
...
func (d *MockManager) GetOEMBMCSettingAttribute(
	ctx context.Context,
...
func (d *MockManager) UpdateBMCAttributesApplyAt(
	ctx context.Context,
...
func (d *MockManager) GetBMCPendingAttributeValues(ctx context.Context) (redfish.SettingsAttributes, error) {
...
func (d *MockManager) CheckBMCAttributes(
	ctx context.Context,
	attributes redfish.SettingsAttributes,
) (bool, error) {
	filteredAttr, err := d.getFilteredBMCRegistryAttributes(ctx, false, false)
