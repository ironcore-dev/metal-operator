...
func (r *ServerReconciler) handleDiscoveryState(ctx context.Context, log logr.Logger, server *metalv1alpha1.Server) (bool, error) {
	...
	for _, storage := range storages {
		a1Storage := metalv1alpha1.Storage{
			Name:  storage.Name,
		...
		for _, volume := range storage.Volumes {
			a1Storage.Volumes = append(a1Storage.Volumes, metalv1alpha1.StorageVolume{
				Name:        volume.Name,
		...
		server.Status.Storages = append(server.Status.Storages, a1Storage)
	}
