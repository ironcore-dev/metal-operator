...
func (r *BiosSettingsReconciler) handleSettingInProgressState(
	...
	// check if the pending tasks if present has correct value
	// at this point pendingSettingsTasks might be present after issuing the setting update.
	// but can not be different from the what is present in settingsDiff
	...
	if len(pendingSettingsDiff) > 0 && pendingPresent {
